...
constexpr int64_t kModeRestartDelayUs = 350LL * 1000LL;
...
void switch_mode_from_button(int64_t now) {
    const OutputMode current = config_output_mode();
    const OutputMode next = next_mode(current);
    config_set_output_mode(next);
    const esp_err_t save_result = config_save();
    if (save_result != ESP_OK) {
        config_set_output_mode(current);
        ESP_LOGE(TAG, "BOOT double-click: failed to save USB mode %s -> %s: %s",
    ...
    s_board.restart_pending = true;
    s_board.restart_at_us = now + kModeRestartDelayUs;
    ESP_LOGW(TAG, "BOOT double-click: switching USB mode %s -> %s in %lld ms",
             config_output_mode_name(current),
             config_output_mode_name(next),
             static_cast<long long>(kModeRestartDelayUs / 1000));

    // Detach before rebooting so the host closes the old personality and does
    // not carry control/interrupt transfers into the next descriptor set.
    if (tud_inited()) {
        tud_disconnect();
    }
}
...
Rgb status_color(int64_t now) {
    if (s_board.restart_pending) {
        return mode_color(config_output_mode());
    }
...
void board_controls_task() {
    ...
    poll_status_led(now);
    if (s_board.restart_pending && now >= s_board.restart_at_us) {
        const bool boot_released = gpio_get_level(
            static_cast<gpio_num_t>(CONFIG_NS2_BOOT_BUTTON_GPIO)) != 0;
        if (boot_released) {
            ESP_LOGW(TAG, "USB personality restart now: %s",
                     config_output_mode_name(config_output_mode()));
            esp_restart();
        }
    }
}
