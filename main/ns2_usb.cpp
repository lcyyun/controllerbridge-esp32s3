...
constexpr uint8_t kNintendoInterfaceCount = 2;
...
constexpr uint8_t kXInputManagerHidEpOut = 0x03;
constexpr uint8_t kXInputManagerHidEpIn = 0x83;
// Manager traffic uses HID feature reports on endpoint zero. Keep its
// otherwise-unused interrupt endpoints small.
constexpr uint8_t kManagerEndpointPacketSize = 8;
...
// Compatibility identity: keep the XUSB20 wire protocol, but do not claim the
// Microsoft VID/PID.  This also lets Windows expose the separate manager HID.
constexpr uint16_t kXInputVid = 0x1209;
constexpr uint16_t kXInputPid = 0x4e53;
...
const tusb_desc_device_t kXInputDeviceDescriptor = {
    ...
    .bMaxPacketSize0 = CFG_TUD_ENDPOINT0_SIZE,
    .idVendor = kXInputVid,
    .idProduct = kXInputPid,
    // Keep this distinct from the legacy/custom identities so Windows does
    // not reuse a cached failed Microsoft OS descriptor probe.
    .bcdDevice = 0x0200,
    .iManufacturer = 0x01,
    .iProduct = 0x02,
    .iSerialNumber = 0x03,
...
const uint8_t kManagerReportDescriptor[] = {
    // Keep this interface compatible with the existing unified WebHID
    // manager: vendor page 0xff00, feature report 0x7f and Y7HID1/Y7HRS1.
    0x06, 0x00, 0xff,
    0x09, 0x01,
    0xa1, 0x01,
    0x15, 0x00, 0x26, 0xff, 0x00,
    0x75, 0x08,
    // A one-byte interrupt input/output pair makes this collection visible to
    // HID clients that omit feature-only collections.  The manager itself
    // continues to use only feature report 0x7f, so the 8-byte endpoint budget
    // of the DualSense configuration is preserved.
    0x85, kUsbReportIdInput,
    0x95, 0x01,
    0x09, 0x01,
    0x81, 0x02,
    0x85, kUsbReportIdOutput,
    0x95, 0x01,
    0x09, 0x01,
    0x91, 0x02,
    0x85, kUsbReportIdFeature,
    0x95, kInputReportPayloadLen,
    0x09, 0x01,
    0xb1, 0x02,
    0xc0,
};
...
const uint8_t kDualSenseReportDescriptor[] = {
    ...
    // A second top-level vendor collection lets WebHID discover the manager
    // on the existing DualSense HID interface, without consuming another pair
    // of USB endpoints (the DS audio FIFO leaves no room for them).
    0x06, 0x00, 0xff, 0x09, 0x01, 0xa1, 0x01,
    0x15, 0x00, 0x26, 0xff, 0x00, 0x75, 0x08,
    0x85, kUsbReportIdFeature, 0x95, kInputReportPayloadLen, 0x09, 0x01,
    0xb1, 0x02, 0xc0,
};
static_assert(sizeof(kDualSenseReportDescriptor) == 0x0158);
...
constexpr uint16_t kConfigTotalLen = TUD_CONFIG_DESC_LEN +
                                     TUD_HID_INOUT_DESC_LEN +
                                     TUD_VENDOR_DESC_LEN;
const uint8_t kConfigurationDescriptor[] = {
    TUD_CONFIG_DESCRIPTOR(1, kNintendoInterfaceCount, 4, kConfigTotalLen, 0, 500),
...
const uint8_t kXInputConfigurationDescriptor[] = {
    ...
    TUD_HID_NS2_INOUT_DESCRIPTOR(1, 4, HID_ITF_PROTOCOL_NONE,
                                 sizeof(kManagerReportDescriptor),
                                 kXInputManagerHidEpIn, kXInputManagerHidEpOut,
                                 kManagerEndpointPacketSize, 4),
};
static_assert(sizeof(kXInputConfigurationDescriptor) == kXInputConfigTotalLen);
...
// Bind only XInput interface 0 to the inbox XUSB driver. Interface 1 remains
// a normal HID child and exposes the complete Manager protocol independently.
const uint8_t kXInputMsOs10CompatIdDescriptor[] = {
    U32_TO_U8S_LE(kMsOs10CompatIdLen),
    U16_TO_U8S_LE(0x0100),
    U16_TO_U8S_LE(0x0004),
    0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static_assert(sizeof(kXInputMsOs10CompatIdDescriptor) == kMsOs10CompatIdLen);
...
bool is_manager_hid_instance(uint8_t instance) {
    return s_usb.output_mode == OutputMode::XInput && instance == kGamepadHidInstance;
}

bool is_gamepad_hid_instance(uint8_t instance) {
    return s_usb.output_mode != OutputMode::XInput && instance == kGamepadHidInstance;
}
...
void usb_task() {
    ...
    if (s_usb.output_mode == OutputMode::XInput) {
        ...
        sent = ns2_xinput_usb_send(report, sizeof(report));
...
extern "C" uint8_t const *tud_hid_descriptor_report_cb(uint8_t instance) {
    if (ns2::is_manager_hid_instance(instance)) {
        return ns2::kManagerReportDescriptor;
    }
    if (ns2::s_usb.output_mode == ns2::OutputMode::DualSense &&
        ns2::is_gamepad_hid_instance(instance)) {
        return ns2::kDualSenseReportDescriptor;
    }
    return ns2::is_gamepad_hid_instance(instance) ? ns2::kHidReportDescriptor : nullptr;
}
...
extern "C" uint16_t tud_hid_get_report_cb(uint8_t instance,
    ...
    if (report_type == HID_REPORT_TYPE_FEATURE &&
        ns2::is_manager_hid_instance(instance) &&
...
extern "C" void tud_hid_report_complete_cb(uint8_t instance, uint8_t const *report, uint16_t len) {
    if (!ns2::is_gamepad_hid_instance(instance)) {
        return;
    }
...
extern "C" uint16_t const *__wrap_tud_descriptor_string_cb(uint8_t index, uint16_t langid) {
    if (index == ns2::kMsOs10StringIndex &&
        (ns2::s_usb.output_mode == ns2::OutputMode::Nintendo ||
         ns2::s_usb.output_mode == ns2::OutputMode::XInput)) {
...
extern "C" bool tud_vendor_control_xfer_cb(uint8_t rhport,
    ...
    if (request->wIndex == 0x0004) {
        ...
        if (ns2::s_usb.output_mode == ns2::OutputMode::Nintendo) {
            return tud_control_xfer(rhport,
                                    request,
                                    const_cast<uint8_t *>(ns2::kMsOs10CompatIdDescriptor),
                                    sizeof(ns2::kMsOs10CompatIdDescriptor));
        }
        if (ns2::s_usb.output_mode == ns2::OutputMode::XInput) {
            return tud_control_xfer(rhport,
                                    request,
                                    const_cast<uint8_t *>(ns2::kXInputMsOs10CompatIdDescriptor),
                                    ns2::kMsOs10CompatIdLen);
        }
        return false;
    }
    if (request->wIndex == 0x0005 && ns2::s_usb.output_mode == ns2::OutputMode::Nintendo) {
    ...
    if (request->wIndex == 0x0007 && ns2::s_usb.output_mode == ns2::OutputMode::Nintendo) {
...
extern "C" void ns2_xinput_output_received(const uint8_t *data, uint16_t length) {
    if (data == nullptr || length == 0 || ns2::s_usb.output_mode != ns2::OutputMode::XInput) {
        return;
    }
    ns2::s_usb.vendor_out_count++;
    ns2::s_usb.vendor_last_rx_len = length;
    ns2::bridge_xinput_output_to_ble(data, length);
}
