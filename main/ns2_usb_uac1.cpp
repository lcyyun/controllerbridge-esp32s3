...
#error "DualSense UAC1 requires TinyUSB slave mode: DMA metadata leaves insufficient FIFO for 384-byte audio OUT plus HID IN"
#endif
...
extern "C" usbd_class_driver_t const *usbd_app_driver_get_cb(uint8_t *driver_count) {
    ...
    if (!initialized) {
        drivers[0] = s_uac1_driver;
        drivers[1] = *ns2_xinput_usb_driver();
        initialized = true;
    }
    *driver_count = 2;
    return drivers;
